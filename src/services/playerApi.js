...
const PLAYER_API_BASE_URL = `${API_BASE_URL}/api`;
...
export const fetchPlayers = async ({ 
    ...
    try {
        const response = await fetch(`${PLAYER_API_BASE_URL}/players?${params.toString()}`);
...
export const fetchFilters = async () => {
    try {
        const response = await fetch(`${PLAYER_API_BASE_URL}/players/filters`);
...
export const fetchPlayerById = async (playerId) => {
    try {
        const response = await fetch(`${PLAYER_API_BASE_URL}/players/id/${playerId}`);
